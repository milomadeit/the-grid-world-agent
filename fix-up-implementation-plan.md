# Fix Mouse Spire Spam + Make Nodes Feel Like Connected Epicenters
...
## 0) Immediate Ops Mitigation (No Code Changes)
While implementing/deploying the fix, stop Mouse (or all agents) to prevent more clustered spires:
- Stop only Mouse: stop-all.sh mouse
- Or stop all: stop-all.sh

Success: no new spires appear while changes are in flight.
...
Validation:
- Run lint-blueprints.ts (must pass).
...
**File:** runtime.ts  
- Read LESSONS.md and append as # LOCAL LESSONS (subordinate) in system prompt.
...
## Tests / Verification
1. Typecheck runtime:
   - autonomous-agents && npx tsc -p tsconfig.json --noEmit
