...
interface AgentConfig {
  ...
  /** ERC-8004 registry URI (e.g., eip155:84532:0x8004...) */
...
interface AgentDecision {
  thought: string;
  action: 'MOVE' | 'CHAT' | 'SEND_DM' | 'BUILD_BLUEPRINT' | 'BUILD_CONTINUE' | 'CANCEL_BUILD' | 'BUILD_PRIMITIVE' | 'BUILD_MULTI' | 'TERMINAL' | 'VOTE' | 'SUBMIT_DIRECTIVE' | 'COMPLETE_DIRECTIVE' | 'TRANSFER_CREDITS' | 'START_CERTIFICATION' | 'SUBMIT_CERTIFICATION_PROOF' | 'CHECK_CERTIFICATION' | 'EXECUTE_SWAP' | 'EXECUTE_ONCHAIN' | 'APPROVE_TOKEN' | 'IDLE';
...
const BLUEPRINT_TIER3_NODE_TIERS = new Set(['city-node', 'metropolis-node', 'megaopolis-node']);
const BLUEPRINT_ARCHITECT_EXCLUSIVE_NODE_TIERS = new Set(['metropolis-node', 'megaopolis-node']);
const BLUEPRINT_ARCHITECT_EXCLUSIVE_MIN_PRIMITIVES = 40;
const AGENT_EVENT_CONTEXT_LIMIT = 40;
const SYSTEM_EVENT_CONTEXT_LIMIT = 25;
...
const BUILD_ACTIONS: ReadonlySet<AgentDecision['action']> = new Set([
  'BUILD_BLUEPRINT',
  'BUILD_CONTINUE',
  'BUILD_PRIMITIVE',
  'BUILD_MULTI',
...
function pickPreferredCertificationTemplateId(certTemplates: any[]): string | null {
  if (!Array.isArray(certTemplates) || certTemplates.length === 0) return null;

  const normalized = certTemplates.map((tpl) => ({
    id: String(tpl?.id || tpl?.templateId || '').trim(),
  }));

  const swap = normalized.find((tpl) => tpl.id.toUpperCase() === 'SWAP_EXECUTION_V1');
  if (swap?.id) return swap.id;

  const first = normalized.find((tpl) => tpl.id.length > 0);
  return first?.id || null;
}

function getActiveCertificationRuns(certRuns: any[]): any[] {
  if (!Array.isArray(certRuns)) return [];
  return certRuns.filter((r: any) => {
    const status = String(r?.status || '').toLowerCase();
    return status === 'created' || status === 'active' || status === 'submitted' || status === 'verifying';
  });
}

function formatCertificationFee(template: any): string {
  const raw =
    template?.fee ??
    template?.feeUsdcAtomic ??
    template?.feeAtomic ??
    template?.fee_usdc_atomic;
  if (raw == null || raw === '') return 'TBD';

  const numeric = Number(raw);
  if (Number.isFinite(numeric) && numeric > 0) {
    const usdc = numeric / 1_000_000;
    return `${usdc.toFixed(usdc >= 1 ? 2 : 4)} USDC`;
  }
...
function isArchitectExclusiveBlueprint(blueprint: any): boolean {
  ...
  if (
    ...
    BLUEPRINT_ARCHITECT_EXCLUSIVE_NODE_TIERS.has(String(blueprint.minNodeTier).toLowerCase())
  ) {
    return true;
  }

  return blueprintPrimitiveCount(blueprint) >= BLUEPRINT_ARCHITECT_EXCLUSIVE_MIN_PRIMITIVES;
}

function isTier3Blueprint(blueprint: any): boolean {
  if (!blueprint || typeof blueprint !== 'object') return false;
  ...
  return (
    (typeof blueprint.minNodeTier === 'string' &&
      BLUEPRINT_TIER3_NODE_TIERS.has(String(blueprint.minNodeTier).toLowerCase())) ||
...
function formatActionUpdateChat(
  ...
  switch (decision.action) {
    ...
    case 'START_CERTIFICATION': {
      ...
      if (thought.length > 15) return thought;
      return `Starting certification: ${tpl}`;
    }
    case 'SUBMIT_CERTIFICATION_PROOF': {
      const thought = (decision.thought || '').split('|')[0].trim();
      if (thought.length > 15) return thought;
      return `Submitting certification proof.`;
    }
    case 'CHECK_CERTIFICATION': {
      return null; // Read-only, no chat needed
    }
    case 'EXECUTE_SWAP': {
      const thought = (decision.thought || '').split('|')[0].trim();
      if (thought.length > 15) return thought;
      return `Executing swap for certification run.`;
    }
    case 'EXECUTE_ONCHAIN': {
      const thought = (decision.thought || '').split('|')[0].trim();
      if (thought.length > 15) return thought;
      return `Executing onchain transaction for certification.`;
    }
    case 'APPROVE_TOKEN': {
      const thought = (decision.thought || '').split('|')[0].trim();
      if (thought.length > 15) return thought;
      return `Approving token for certification.`;
    }
    default:
      return null;
  }
...
/** Return the full nearest server node object at a position (for maturity/category analysis). */
function closestServerNodeAtPosition(
  serverSpatial: { nodes?: Array<Record<string, any>> } | null,
  x: number,
  z: number,
): {
  ...
  return {
    ...
    structureCount: Number(best.structureCount) || 0,
    dominantCategory: String(best.dominantCategory || 'mixed').toLowerCase(),
    missingCategories: Array.isArray(best.missingCategories)
      ? best.missingCategories.map((c: any) => String(c).toLowerCase())
      : [],
    tier: String(best.tier || '').toLowerCase(),
  };
}
...
export async function startAgent(config: AgentConfig): Promise<void> {
  ...
  // Derive agent role key for per-agent blueprint/spatial scoring
  const agentRole = lowerAgentName.includes('smith') ? 'smith'
    : lowerAgentName.includes('oracle') ? 'oracle'
    : lowerAgentName.includes('clank') ? 'clank'
    : lowerAgentName.includes('mouse') ? 'mouse'
    : '';
  ...
  const systemPrompt = [
    ...
    'Certifications are the primary value activity. Building is the expression of earned value. Prioritize certification, then build with earned credits.',
    ...
    'Attribution guard: NEVER claim another agent\'s action as your own. Only say "I built/completed/started X" if your own last action detail confirms it.',
    '\n---\n',
    '# LONG-TERM MEMORY\n',
  ...
  try {
    ...
    if (isOwnershipDenied && config.privateKey) {
      ...
      try {
        ...
        console.log(`[${agentName}] Wallet balance: ${(Number(balance) / 1e18).toFixed(4)} ETH`);
        if (balance > BigInt(0)) {
          ...
          console.error(`[${agentName}] No ETH for gas. Fund wallet ${chain.getAddress()} and restart.`);
          return;
        }
      } catch (regErr) {
  ...
  // Create ChainClient for swap execution (persists across ticks)
  const agentChain = config.privateKey ? new ChainClient(config.privateKey) : null;
  if (agentChain) {
    console.log(`[${agentName}] ChainClient initialized for wallet ${agentChain.getAddress()}`);
  }
  ...
  const ACTION_FORMAT_BLOCK = [
    ...
    '{ "thought": "...", "action": "MOVE|CHAT|SEND_DM|BUILD_BLUEPRINT|BUILD_CONTINUE|CANCEL_BUILD|BUILD_PRIMITIVE|BUILD_MULTI|TERMINAL|VOTE|SUBMIT_DIRECTIVE|COMPLETE_DIRECTIVE|TRANSFER_CREDITS|START_CERTIFICATION|EXECUTE_ONCHAIN|APPROVE_TOKEN|SUBMIT_CERTIFICATION_PROOF|CHECK_CERTIFICATION|IDLE", "payload": {...} }',
    '',
    'Payload formats:',
    '  MOVE: {"x": 5, "z": 3}',
    '  CHAT: {"message": "Hello!"}',
    ...
    '  START_CERTIFICATION: {"templateId": "SWAP_EXECUTION_V1"}  ← pay fee + start a certification run',
    '  EXECUTE_ONCHAIN: {"runId": "uuid", "to": "0xContractAddress", "data": "0xCalldata", "value": "0"}  ← sign and send a transaction from your wallet for a certification run. You build the calldata. You handle approvals. You choose the contract.',
    '  APPROVE_TOKEN: {"token": "0xTokenAddress", "spender": "0xSpenderAddress", "amount": "1000000"}  ← approve an ERC-20 token for a spender address. Returns tx hash.',
    '  SUBMIT_CERTIFICATION_PROOF: {"runId": "uuid", "txHash": "0x..."}  ← submit your tx hash for grading',
    '  CHECK_CERTIFICATION: {}  ← view your certification runs and status (read-only)',
    '  IDLE: {}',
    '',
  ...
  const tickKeepaliveSeconds = readEnvNumber('AGENT_TICK_KEEPALIVE_SECONDS', 20, 0);
  const llmTimeoutMs = readEnvNumber('AGENT_LLM_TIMEOUT_MS', 120_000, 5_000);
  console.log(`[${agentName}] Tick keepalive: ${tickKeepaliveSeconds > 0 ? `${tickKeepaliveSeconds}s` : 'disabled'}`);
  console.log(`[${agentName}] LLM timeout: ${Math.round(llmTimeoutMs / 1000)}s`);
  ...
  const tick = async () => {
    ...
    const runTickKeepalive = async () => {
      if (keepaliveInFlight) return;
      keepaliveInFlight = true;
      try {
        await api.getStateLite();
      } catch {
        try {
          await api.getAgentsLite();
        } catch {
          // Best-effort presence touch; ignore transient failures.
        }
      } finally {
        keepaliveInFlight = false;
      }
    };
    const stopTickKeepalive = () => {
      if (keepaliveTimer) {
    ...
    try {
      ...
      localTickCount++;
      if (tickKeepaliveSeconds > 0) {
        keepaliveTimer = setInterval(() => {
          void runTickKeepalive();
        }, tickKeepaliveSeconds * 1000);
      }
      ...
      // Fetch certification state for economy context
      let certTemplates: Awaited<ReturnType<typeof api.getCertificationTemplates>> = [];
      let certRuns: Awaited<ReturnType<typeof api.getCertificationRuns>> = [];
      try {
        [certTemplates, certRuns] = await Promise.all([
          api.getCertificationTemplates(),
          api.getCertificationRuns(),
        ]);
      } catch {
        // Non-critical — certification context will be empty
      }

      let unreadDirectMessages: DirectMessage[] = [];
      try {
        unreadDirectMessages = await api.getInbox(true);
      } catch {
      ...
      console.log(`[${agentName}] State: ${world.agents.length} agents, ${(world.events||[]).length} events, ${directives.length} directives`);
      ...
      const combinedReputation = Math.max(0, Number(self?.combinedReputation ?? 0) || 0);
      const agentClass = String(self?.agentClass || 'builder').toLowerCase();
      ...
      // --- Node-level build tracking (reflection phase) ---
      const currentNearestNode = self
        ? closestServerNodeAtPosition(serverSpatial, self.position.x, self.position.z)
        : null;
      const currentNodeName = currentNearestNode?.name || '';
      const prevNodeName = workingMemory.match(/Current node: (.+)/)?.[1]?.trim() || '';
      const prevTicksAtNode = parseInt(workingMemory.match(/Ticks at node: (\d+)/)?.[1] || '0');
      const prevNodeBuildsCompleted = parseInt(workingMemory.match(/Node builds completed: (\d+)/)?.[1] || '0');
      const prevNodeBuildsAttempted = parseInt(workingMemory.match(/Node builds attempted: (\d+)/)?.[1] || '0');
      const prevNodeBlueprintTypes = (workingMemory.match(/Node blueprint types: (.+)/)?.[1] || '')
        .split(',').map(s => s.trim().toUpperCase()).filter(Boolean);

      const nodeChanged = currentNodeName !== '' && currentNodeName !== prevNodeName && prevNodeName !== '';
      const ticksAtCurrentNode = nodeChanged ? 1 : prevTicksAtNode + 1;
      let nodeBuildsCompleted = nodeChanged ? 0 : prevNodeBuildsCompleted;
      let nodeBuildsAttempted = nodeChanged ? 0 : prevNodeBuildsAttempted;
      let nodeBlueprintTypes = nodeChanged ? [] as string[] : [...prevNodeBlueprintTypes];

      // Detect blueprint completion/abandonment between ticks
      const prevBuildPlanState = workingMemory.match(/Current build plan: (.+)/)?.[1] || '';
      const prevHadActivePlan = prevBuildPlanState.startsWith('Blueprint:');
      const currentHasActivePlan = Boolean(blueprintStatus?.active);
      const lastActionFromMemory = workingMemory.match(/Last action: (.+)/)?.[1]?.trim() || '';
      const prevConsecutiveSameAction = parseInt(
        workingMemory.match(/Consecutive same-action: (\d+)/)?.[1] || '0',
        10,
      );
      const lastActionErrorFromMemory = workingMemory.match(/Last build error: (.+)/)?.[1] || '';
      const certPaymentBlockedFromMemory =
        lastActionFromMemory === 'START_CERTIFICATION' &&
        /x402|insufficient_funds|insufficient funds|missing_eip712_domain|usdc|rate limit exceeded|retryafterms|429/i.test(lastActionErrorFromMemory);

      if (!prevHadActivePlan && currentHasActivePlan) {
        nodeBuildsAttempted++;
      }
      if (prevHadActivePlan && !currentHasActivePlan && lastActionFromMemory !== 'CANCEL_BUILD') {
        nodeBuildsCompleted++;
      }
      if (currentHasActivePlan && blueprintStatus?.blueprintName) {
        const bpName = String(blueprintStatus.blueprintName).toUpperCase();
        if (!nodeBlueprintTypes.includes(bpName)) {
          nodeBlueprintTypes.push(bpName);
        }
      }
      ...
      // Unified event feed — keep a permissive context window while preserving token budget.
      const allEvents = world.events || [];
      const agentEvents = allEvents.filter(e => e.source === 'agent');
      const systemEventsList = allEvents.filter(e => e.source === 'system');
      const recentAgentChat = agentEvents
        .sort((a, b) => a.createdAt - b.createdAt)
        .slice(-AGENT_EVENT_CONTEXT_LIMIT);
      const recentSystem = systemEventsList
        .sort((a, b) => a.createdAt - b.createdAt)
        .slice(-SYSTEM_EVENT_CONTEXT_LIMIT);

      const allChatMessages = [...recentAgentChat, ...recentSystem]
        .sort((a, b) => a.createdAt - b.createdAt)
        .map(e => ({ id: e.id, agentId: e.agentId || '', agentName: e.agentName || (e.source === 'system' ? 'System' : ''), message: e.body, createdAt: e.createdAt }));
      ...
      const userPrompt = [
        ...
        `Your class: ${agentClass}`,
        `Your reputation: ${combinedReputation} (need 5 for directives, 15 for tier-3 blueprints)`,
        'Operating mode: certify → earn → build. Prioritize certifications, then use earned credits for building.',
        '',
        '## UNREAD DMs (private)',
        unreadDirectMessages.length > 0
          ? unreadDirectMessages
              .slice(0, 10)
              .map((dm) => `- [DM from ${dm.fromId}${dm.fromType === 'human' ? ' (human)' : ' (agent)'}]: ${dm.message}`)
              .join('\n')
          : '_No unread DMs._',
        '',
        `## RECENT CHAT (agent + system feed; up to ${AGENT_EVENT_CONTEXT_LIMIT + SYSTEM_EVENT_CONTEXT_LIMIT} messages)`,
        ...
        urgentChat
          ? `**URGENT:** Another agent mentioned you or there's coordination news. Prefer CHAT this tick and respond directly with concrete next steps.`
          : chatDue
            ? `CHAT is recommended this tick if you have meaningful updates or coordination asks. Keep it concise and outcome-oriented.`
            : `Ticks since last chat: ${currentTicksSinceChat}/${chatMinTicks}. No forced chat right now; focus on high-value actions.`,
        '',
        '## Build Variety Guard',
        ...
        directives.length > 0
          ...
          : combinedReputation >= 5
            ...
            : [
                '_No active directives right now._',
                `You need 5 reputation to submit directives (current: ${combinedReputation}). Complete certifications and other value actions (for example trades/transfers) to earn reputation.`,
              ].join('\n'),
        '',
        // --- Certification & Reputation Economy ---
        '## Certification & Reputation Economy',
        'Certifications are the PRIMARY value activity. Complete onchain work to earn reputation and credits.',
        '',
        certTemplates.length > 0
          ? [
              '**Available Certification Templates:**',
              ...certTemplates.map((t: any) => `- **${t.id || t.templateId}**: ${t.description || t.name || 'Certification'} (fee: ${formatCertificationFee(t)})`),
            ].join('\n')
          : '_No certification templates available yet._',
        '',
        (() => {
          const activeRuns = getActiveCertificationRuns(certRuns);
          const passedRuns = certRuns.filter((r: any) => r.status === 'passed');
          const failedRuns = certRuns.filter((r: any) => r.status === 'failed');
          const expiredRuns = certRuns.filter((r: any) => r.status === 'expired');
          const submittedRuns = certRuns.filter((r: any) => r.status === 'submitted' || r.status === 'verifying');
          const lines: string[] = [];
          for (const r of activeRuns) {
            const ch = (r as any).challenge || {};
            lines.push(`**Run ${r.id} [${r.templateId}] — ${ch.network || 'unknown'} cert**`);
            if (ch.objective) lines.push(`Objective: ${ch.objective}`);
            if (ch.deadline) lines.push(`Deadline: ${ch.deadline}`);
            if (Array.isArray(ch.rubric)) {
              lines.push('Rubric:');
              for (const dim of ch.rubric) {
                lines.push(`- [${dim.weight}%] ${dim.dimension}: ${dim.description}`);
              }
            }
            if (ch.passingScore) lines.push(`Passing score: ${ch.passingScore}/100`);
            if (Array.isArray(ch.tools)) {
              lines.push('Available tools:');
              for (const tool of ch.tools) {
                lines.push(`- ${tool}`);
              }
            }
            if (ch.hintsEnabled && ch.hints) {
              lines.push('Hints:');
              if (ch.hints.testnet) lines.push(`- ${ch.hints.testnet}`);
              if (ch.hints.note) lines.push(`- ${ch.hints.note}`);
            }
            if (ch.submission) {
              lines.push(`Submit: ${ch.submission.endpoint}`);
              lines.push(`Body: ${ch.submission.body}`);
            }
          }
          for (const r of submittedRuns) {
            lines.push(`Run ${r.id}: WAITING — verification in progress.`);
          }
          if (passedRuns.length > 0) {
            lines.push(`**Passed:** ${passedRuns.length} certification(s).`);
          }
          for (const r of failedRuns) {
            lines.push(`Run ${r.id}: FAILED.`);
          }
          for (const r of expiredRuns) {
            lines.push(`Run ${r.id}: EXPIRED.`);
          }
          if (certRuns.length === 0) {
            lines.push('_No certification runs. Use START_CERTIFICATION to begin._');
          }
          return lines.join('\n');
        })(),
        '',
        ...
        ...(() => {
          ...
          const scored = safeSpotCandidates.map((spot) => {
            ...
            if (agentRole === 'smith') {
              // SMITH: Guild leader — stay and densify the current node, cluster with guild
              const targetDist = nodeIsDense ? 75 : 50;
              score = Math.abs(distFromAgent - targetDist);

              // Strong growth preference — Smith is the anchor, always densifying
              if (spot.type === 'growth') score -= nodeIsDense ? 22 : 48;
              if (spot.type === 'connector') score += nodeIsEstablished ? (nodeIsDense ? -8 : 0) : 12;
              if (spot.type === 'frontier') {
                if (!nodeIsEstablished) score += 80; // hard-block frontier before 25
                else if (!nodeIsDense) score += 25; // strong soft-block before 50
                else score -= 4;
              }
              ...
              // Smith stays very close until dense — the guild leader doesn't wander
              if (!nodeIsDense && spot.nearestBuild >= NODE_EXPANSION_MIN_DISTANCE) score += 28;
              if (!nodeIsEstablished && distFromAgent > 100) score += 25;

              // Strong clustering bonus — Smith wants to be with the guild
              const nearestGuildAgent = otherAgents
                .filter(a => ['clank','oracle'].some(n => a.name.toLowerCase().includes(n)))
                .reduce((min, a) => Math.min(min, Math.hypot(spot.x - a.position.x, spot.z - a.position.z)), 999);
              if (nearestGuildAgent < 40) score -= 20;

            } else if (agentRole === 'oracle') {
              // ORACLE: Infrastructure planner — prefers connector zones, unlocks frontier earlier
              const targetDist = nodeIsDense ? 90 : 60;
              score = Math.abs(distFromAgent - targetDist);

              // Oracle still likes growth but STRONGLY prefers connector spots for roads
              if (spot.type === 'growth') score -= nodeIsDense ? 15 : 25;
              if (spot.type === 'connector') score -= nodeIsEstablished ? 38 : 8; // STRONG connector preference once node established
              if (spot.type === 'frontier') {
                if (!nodeIsEstablished) score += 50; // block frontier before 25 (softer than Smith)
                else if (!nodeIsDense) score += 6; // Oracle can start connecting earlier
                else score -= 12; // Oracle actively explores to connect distant nodes
              }

              // Keep spacing aligned with frontier lane window.
              if (spot.nearestBuild < NODE_EXPANSION_MIN_DISTANCE && spot.type === 'frontier') score += 15;
              if (spot.nearestBuild > NODE_EXPANSION_MAX_DISTANCE + 1) score += 15;

              // Oracle has wider range — needs to reach between nodes for infrastructure
              if (!nodeIsDense && spot.nearestBuild >= NODE_EXPANSION_MIN_DISTANCE) score += 12;
              if (!nodeIsEstablished && distFromAgent > 150) score += 15;

              // Mild clustering — Oracle coordinates but doesn't need to be right on top of guild
              const nearestGuildAgent = otherAgents
                .filter(a => ['smith','clank'].some(n => a.name.toLowerCase().includes(n)))
                .reduce((min, a) => Math.min(min, Math.hypot(spot.x - a.position.x, spot.z - a.position.z)), 999);
              if (nearestGuildAgent < 60) score -= 8;

            } else if (agentRole === 'clank') {
              // CLANK: Rapid builder / explorer — wider range, moves between nodes faster
              const targetDist = nodeIsDense ? 85 : 55;
              score = Math.abs(distFromAgent - targetDist);

              // Clank likes growth but is more willing to move to new spots
              if (spot.type === 'growth') score -= nodeIsDense ? 18 : 32;
              if (spot.type === 'connector') score += nodeIsEstablished ? (nodeIsDense ? -12 : -4) : 8;
              if (spot.type === 'frontier') {
                if (!nodeIsEstablished) score += 55; // softer frontier block than Smith
                else if (!nodeIsDense) score += 8; // Clank unlocks frontier earlier than Smith
                else score -= 14; // Clank actively seeks new areas once dense
              }

              // Keep spacing aligned with frontier lane window.
              if (spot.nearestBuild < NODE_EXPANSION_MIN_DISTANCE && spot.type === 'frontier') score += 18;
              if (spot.nearestBuild > NODE_EXPANSION_MAX_DISTANCE + 1) score += 18;

              // Clank has wider roaming range — the explorer of the guild
              if (!nodeIsDense && spot.nearestBuild >= NODE_EXPANSION_MIN_DISTANCE) score += 14;
              if (!nodeIsEstablished && distFromAgent > 140) score += 12;

              // Light clustering — Clank helps the guild but is more independent
              const nearestGuildAgent = otherAgents
                .filter(a => ['smith','oracle'].some(n => a.name.toLowerCase().includes(n)))
                .reduce((min, a) => Math.min(min, Math.hypot(spot.x - a.position.x, spot.z - a.position.z)), 999);
              if (nearestGuildAgent < 40) score -= 10;

            } else if (isGuildBuilderAgent) {
              // Fallback for any unrecognized guild agent
              const targetDist = nodeIsDense ? 75 : 50;
              score = Math.abs(distFromAgent - targetDist);
              if (spot.type === 'growth') score -= nodeIsDense ? 22 : 42;
              if (spot.type === 'connector') score += nodeIsEstablished ? (nodeIsDense ? -10 : -2) : 10;
              if (spot.type === 'frontier') {
                if (!nodeIsEstablished) score += 70;
                else if (!nodeIsDense) score += 18;
                else score -= 6;
              }
              if (spot.nearestBuild < NODE_EXPANSION_MIN_DISTANCE && spot.type === 'frontier') score += 20;
              if (spot.nearestBuild > NODE_EXPANSION_MAX_DISTANCE + 1) score += 20;
              ...
              const nearestGuildAgent = otherAgents
                ...
                .reduce((min, a) => Math.min(min, Math.hypot(spot.x - a.position.x, spot.z - a.position.z)), 999);
              if (nearestGuildAgent < 40) score -= 15;
        ...
        ...(workingMemory ? (() => {
          const lastAction = workingMemory.match(/Last action: (\w+)/)?.[1];
          if (lastAction === 'SUBMIT_DIRECTIVE' && combinedReputation < 5) {
            return [
              `**BLOCKED:** You need 5 reputation to submit directives (current: ${combinedReputation}). **DO NOT try again. CHAT, coordinate, transfer credits, or start a CERTIFICATION instead.**`,
              '',
            ];
          }
          return [];
        ...
        ACTION_FORMAT_BLOCK,
        // Blueprint section — show active plan or catalog
              ...
              (() => {
                ...
                if (!cachedBlueprintCatalog || blueprintCacheTickCount >= BLUEPRINT_CACHE_REFRESH_INTERVAL) {
                  ...
                  cachedBlueprintCatalog = [
                    ...
                    '  BUILD_BLUEPRINT: {"name":"DATACENTER","anchorX":120,"anchorZ":120,"rotY":90}',
                    '',
                    '**ROTATION (rotY):** ALWAYS set rotY to orient your builds. 0=north, 90=east, 180=south, 270=west.',
                    'Vary rotations across builds for visual interest. Face entrances toward paths, plazas, or neighboring structures.',
                    'Avoid placing 3+ structures at the same angle — alternate between 0, 90, 180, 270.',
                    `YOUR CLASS: ${agentClass} — blueprints marked ARCHITECT-ONLY require architect class.`,
                    '',
                    ...Object.entries(blueprints).map(([name, bp]: [string, any]) => {
                      const totalPrimitives = blueprintPrimitiveCount(bp);
                      const estTicks = Math.max(1, Math.ceil(totalPrimitives / 5));
                      ...
                      if (isArchitectExclusiveBlueprint(bp)) gateNotes.push('**[ARCHITECT-ONLY]**');
                      if (isTier3Blueprint(bp)) gateNotes.push('**[NEEDS 15 REP]**');
                      const gateLabel = gateNotes.length > 0 ? ` | ${gateNotes.join(' ')}` : '';
                      return `- **${name}**${catLabel} — ${bp.description || 'Blueprint'} | ${totalPrimitives} pieces, ~${estTicks} ticks | ${bp.difficulty || 'unknown'}${tierNote}${tagsLabel}${gateLabel}`;
                    }),
                    '',
      ...
      const directivePolicyDecision: AgentDecision | null = (() => {
        ...
        if (directives.length === 0 && lowerAgentName === 'smith' && combinedReputation >= 5) {
          ...
          const description = '[Certification Push] Coordinate certification starts/submissions and unblock x402 payment path. Share blockers, assign owners, and report outcomes.';

          const thought = pickRandom([
            'We need a shared economic objective. I\'m proposing a certification push for the group.',
            'No active directives, so I\'m setting a certification coordination goal.',
            'Let\'s get organized around outcomes. Submitting a certification directive now.',
            'Proposing a group directive focused on certification throughput and payment unblock.',
          ]);
      ...
      if (!blueprintStatus?.active && priorConsecutiveBuildFails >= 6) {
        ...
        const moveTarget = escapeTarget || chooseLocalMoveTarget(
          self?.position ? { x: self.position.x, z: self.position.z } : undefined,
          safeSpots,
          80,
        );
      ...
      } else if (skipLLMForUnchangedState && !shouldReflect) {
        if (blueprintStatus?.active) {
          ...
          if (agentRole === 'mouse' && (nearestNodeStructuresAtSelf ?? 0) >= NODE_STRONG_DENSITY_TARGET) {
            const moveTarget = chooseLocalMoveTarget(selfPosForPolicy, safeSpots, 80);
            if (moveTarget) {
              decision = {
          ...
          if (!roleOverride) {
            // Steady state — no world changes detected. Let agent idle or take a local move.
            const localMove = chooseLocalMoveTarget(selfPosForPolicy, safeSpots, 80);
            if (localMove) {
              decision = {
                thought: `Steady state: repositioning to (${localMove.x}, ${localMove.z}).`,
                action: 'MOVE',
                payload: localMove,
              };
            } else {
        ...
        // Reflection gate — enrich prompt when reflection is due
        if (shouldReflect) {
          reflectionThisTick = true;
          lastReflectionTick = localTickCount;
        ...
        const basePrompt = visualSummary
          ? `${userPrompt}\n\n## VISUAL SUMMARY (Gemini)\n${visualSummary}`
          : userPrompt;
        const llmInputPrompt = basePrompt + reflectionSection;
        ...
        try {
          const llmResponse = await withTimeout(
            callLLM(config, fullSystemPrompt, modelPrompt, imageBase64),
            llmTimeoutMs,
      ...
      // Chat message — trust LLM output directly
      if (decision.action === 'CHAT') {
        const rawMessage = String((decision.payload as any)?.message || '').trim();
        if (rawMessage) {
          decision.payload = { ...(decision.payload || {}), message: rawMessage };
        }
      }
      ...
      let buildError = await executeAction(api, agentName, decision, self?.position ? { x: self.position.x, z: self.position.z } : undefined, agentChain || undefined);
      if (buildError) {
        ...
        if (parsedError.expansionGate) {
          ...
          if (densifySpot) {
            ...
            const chosenBlueprint =
              requestedName && requestedBlueprint && !requestedBlueprint.advanced
                ? requestedName
                : fallbackBlueprint;
            const densifyRotY = FALLBACK_ROTATIONS[Math.floor(Math.random() * FALLBACK_ROTATIONS.length)];
            const retryDecision: AgentDecision =
              selfPos && densifyDistance > 20
                ...
                : chosenBlueprint
                  ? {
                      thought: `Expansion gate: new node blocked near "${parsedError.gateNodeName || 'nearest node'}" (${parsedError.gateNodeStructures ?? '?'} structures). Densifying at (${densifySpot.x}, ${densifySpot.z}) with ${chosenBlueprint} rot=${densifyRotY}.`,
                      action: 'BUILD_BLUEPRINT',
                      payload: { name: chosenBlueprint, anchorX: densifySpot.x, anchorZ: densifySpot.z, rotY: densifyRotY },
                    }
                  : {
                      thought: `Expansion gate: new node blocked near "${parsedError.gateNodeName || 'nearest node'}" (${parsedError.gateNodeStructures ?? '?'} structures). Moving to densify lane (${densifySpot.x}, ${densifySpot.z}).`,
                      action: 'MOVE',
            ...
            const retryError = await executeAction(api, agentName, retryDecision, selfPos, agentChain || undefined);
            if (!retryError) {
          ...
          if (fallbackBlueprint && fallbackAnchor) {
            const anchorDistance = selfPos
              ...
              : 0;
            const recoveryRotY = FALLBACK_ROTATIONS[Math.floor(Math.random() * FALLBACK_ROTATIONS.length)];
            const retryDecision: AgentDecision =
              selfPos && anchorDistance > 20
                ...
                : {
                    thought: `Build recovery: continue failed due no active plan; starting fallback blueprint rot=${recoveryRotY}.`,
                    action: 'BUILD_BLUEPRINT',
                    payload: {
      ...
      const actionSummary = decision.action === 'CHAT' ? `CHAT: "${(decision.payload as any)?.message?.slice(0, 80) || ''}"`
        : decision.action === 'SEND_DM' ? `SEND_DM: to ${(decision.payload as any)?.toAgentId || '?'} "${(decision.payload as any)?.message?.slice(0, 80) || ''}"`
        ...
        : decision.action === 'EXECUTE_SWAP' ? `EXECUTE_SWAP (deprecated): run=${(decision.payload as any)?.runId || '?'}`
        : decision.action === 'EXECUTE_ONCHAIN' ? `EXECUTE_ONCHAIN: to=${(decision.payload as any)?.to?.slice(0, 12) || '?'} data=${(decision.payload as any)?.data?.slice(0, 10) || '?'}${(decision as any)._txHash ? ` tx=${(decision as any)._txHash}` : ''}`
        : decision.action === 'APPROVE_TOKEN' ? `APPROVE_TOKEN: token=${(decision.payload as any)?.token?.slice(0, 12) || '?'} spender=${(decision.payload as any)?.spender?.slice(0, 12) || '?'} amt=${(decision.payload as any)?.amount || '?'}${(decision as any)._txHash ? ` tx=${(decision as any)._txHash}` : ''}`
      ...
      // Track node-level blueprint types from this tick's decision
      if (!buildError && decision.action === 'BUILD_BLUEPRINT') {
        const bpName = String((decision.payload as any)?.name || '').trim().toUpperCase();
        if (bpName && !nodeBlueprintTypes.includes(bpName)) {
          nodeBlueprintTypes.push(bpName);
        }
      }
      ...
      if (unreadDirectMessages.length > 0) {
        try {
          const dmIds = unreadDirectMessages.map((dm) => dm.id);
          await api.markDMsRead(dmIds);
        } catch (err) {
          const dmErr = err instanceof Error ? err.message : String(err);
          console.warn(`[${agentName}] Failed to mark DMs read: ${dmErr.slice(0, 140)}`);
        }
      }
      ...
      const newWorking = [
        ...
        ticksAtCurrentNode > 0 ? `Ticks at node: ${ticksAtCurrentNode}` : '',
        nodeBuildsCompleted > 0 ? `Node builds completed: ${nodeBuildsCompleted}` : '',
        nodeBuildsAttempted > 0 ? `Node builds attempted: ${nodeBuildsAttempted}` : '',
        nodeBlueprintTypes.length > 0 ? `Node blueprint types: ${nodeBlueprintTypes.join(', ')}` : '',
      ...
      const reflectLogPrefix = reflectionThisTick ? '[REFLECT] ' : '';
      appendLog(dailyLogPath, `[${timestamp()}] ${reflectLogPrefix}${decision.action}: ${decision.thought}`);
...
export async function bootstrapAgent(config: BootstrapConfig): Promise<void> {
  ...
  const bootstrapLlmTimeoutMs = readEnvNumber('AGENT_LLM_TIMEOUT_MS', 120_000, 5_000);
  const bootstrapTickKeepaliveSeconds = readEnvNumber('AGENT_TICK_KEEPALIVE_SECONDS', 20, 0);
  ...
  if (config.privateKey) {
    ...
    try {
      ...
      log(`Balance: ${balMon.toFixed(4)} ETH`);
    } catch (err) {
  ...
  const bootstrapPrompt = [
    ...
    `Balance: ${Number(walletBalance) / 1e18} ETH`,
    '',
    '## What You Need To Do',
    `Call register() on the IdentityRegistry at ${config.erc8004Registry} to mint your agent ID.`,
    'You have a private key and ETH for gas. You CAN do this yourself.',
  ...
  try {
    const llmResponse = await withTimeout(
      callLLM(config, systemPrompt, bootstrapPrompt),
      bootstrapLlmTimeoutMs,
      `LLM timeout after ${Math.round(bootstrapLlmTimeoutMs / 1000)}s`
    );
  ...
  if (shouldRegister && chain && walletBalance > BigInt(0)) {
    ...
    log('Step 5: SKIPPED — wallet has no ETH for gas.');
  } else {
  ...
  if (newAgentId !== null && chain) {
    ...
    try {
      ...
      const tick = async () => {
        ...
        let keepaliveTimer: ReturnType<typeof setInterval> | null = null;
        let keepaliveInFlight = false;
        const runBootstrapKeepalive = async () => {
          if (keepaliveInFlight) return;
          keepaliveInFlight = true;
          try {
            await api.getStateLite();
        ...
        try {
          if (bootstrapTickKeepaliveSeconds > 0) {
            keepaliveTimer = setInterval(() => {
              void runBootstrapKeepalive();
            }, bootstrapTickKeepaliveSeconds * 1000);
          }
          ...
          // Fetch certification state for economy context
          let bsCertTemplates: Awaited<ReturnType<typeof api.getCertificationTemplates>> = [];
          let bsCertRuns: Awaited<ReturnType<typeof api.getCertificationRuns>> = [];
          try {
            [bsCertTemplates, bsCertRuns] = await Promise.all([
              api.getCertificationTemplates(),
              api.getCertificationRuns(),
            ]);
          } catch {
          ...
          // Unified event feed — keep a permissive context window while preserving token budget.
          const bsAllEvents = world.events || [];
          const bsAgentEvents = bsAllEvents.filter(e => e.source === 'agent');
          const bsSystemEvents = bsAllEvents.filter(e => e.source === 'system');
          const bsRecentAgentChat = bsAgentEvents
            .sort((a, b) => a.createdAt - b.createdAt)
            .slice(-AGENT_EVENT_CONTEXT_LIMIT);
          const bsRecentSystem = bsSystemEvents
            .sort((a, b) => a.createdAt - b.createdAt)
            .slice(-SYSTEM_EVENT_CONTEXT_LIMIT);
          ...
          const userPrompt = [
            ...
            '',
            // --- Certification & Reputation Economy (bootstrap) ---
            '## Certification & Reputation Economy',
            'Certifications are the PRIMARY value activity. Complete onchain work to earn reputation and credits.',
            '',
            bsCertTemplates.length > 0
              ? [
                  '**Available Certification Templates:**',
                  ...bsCertTemplates.map((t: any) => `- **${t.id || t.templateId}**: ${t.description || t.name || 'Certification'} (fee: ${formatCertificationFee(t)})`),
                ].join('\n')
              : '_No certification templates available yet._',
            '',
            (() => {
              const bsActiveRuns = getActiveCertificationRuns(bsCertRuns);
              const bsPassedRuns = bsCertRuns.filter((r: any) => r.status === 'passed');
              const bsFailedRuns = bsCertRuns.filter((r: any) => r.status === 'failed');
              const bsExpiredRuns = bsCertRuns.filter((r: any) => r.status === 'expired');
              const bsSubmittedRuns = bsCertRuns.filter((r: any) => r.status === 'submitted' || r.status === 'verifying');
              const lines: string[] = [];
              for (const r of bsActiveRuns) {
                const ch = (r as any).challenge || {};
                lines.push(`**Run ${r.id} [${r.templateId}] — ${ch.network || 'unknown'} cert**`);
                if (ch.objective) lines.push(`Objective: ${ch.objective}`);
                if (ch.deadline) lines.push(`Deadline: ${ch.deadline}`);
              ...
              for (const r of bsSubmittedRuns) {
                lines.push(`Run ${r.id}: WAITING — verification in progress.`);
              }
              if (bsPassedRuns.length > 0) {
                lines.push(`**Passed:** ${bsPassedRuns.length} certification(s).`);
              }
              for (const r of bsFailedRuns) {
                lines.push(`Run ${r.id}: FAILED.`);
              }
              for (const r of bsExpiredRuns) {
                lines.push(`Run ${r.id}: EXPIRED.`);
              }
              if (bsCertRuns.length === 0) {
                lines.push('_No certification runs. Use START_CERTIFICATION to begin._');
              }
              return lines.join('\n');
            })(),
            ...
            'Decide your next action. Respond with EXACTLY one JSON object:',
            '{ "thought": "...", "action": "MOVE|CHAT|SEND_DM|BUILD_BLUEPRINT|BUILD_CONTINUE|CANCEL_BUILD|BUILD_PRIMITIVE|BUILD_MULTI|TERMINAL|VOTE|SUBMIT_DIRECTIVE|START_CERTIFICATION|EXECUTE_ONCHAIN|APPROVE_TOKEN|SUBMIT_CERTIFICATION_PROOF|CHECK_CERTIFICATION|IDLE", "payload": {...} }',
            '',
            'Payload formats:',
            '  MOVE: {"x": 5, "z": 3}',
            '  CHAT: {"message": "Hello!"}',
          ...
          const llmInputPrompt = visualSummary
            ...
            : userPrompt;
          const llmResponse = await withTimeout(
            callLLM(fullConfig, fullSystemPrompt, llmInputPrompt, imageBase64),
            bootstrapLlmTimeoutMs,
            `LLM timeout after ${Math.round(bootstrapLlmTimeoutMs / 1000)}s`
          );
          const raw = llmResponse.text;
          ...
          console.log(`[${agentName}] ${decision.thought} -> ${decision.action}`);
          const bsBuildError = await executeAction(api, agentName, decision, self?.position ? { x: self.position.x, z: self.position.z } : undefined, chain || undefined);
          if (bsBuildError) {
  ...
  try {
    switch (decision.action) {
      ...
      case 'CHAT':
        ...
        const MAX_LEN = 280;
        const msgToSend = rawMsg.length <= MAX_LEN
          ? rawMsg
          : `${rawMsg.slice(0, MAX_LEN - 3).trimEnd()}...`;
        if (rawMsg.length > MAX_LEN) {
          console.log(`[${name}] Truncating long chat (${rawMsg.length} -> ${msgToSend.length} chars) to avoid cadence spam.`);
        }
        await api.action('CHAT', { message: msgToSend });
        console.log(`[${name}] Sent chat: "${msgToSend.slice(0, 50)}..."`);
        break;
      ...
      case 'SEND_DM': {
        ...
        if (!toAgentId || !dmMessage) {
          console.warn(`[${name}] SEND_DM requires toAgentId and message. Skipping.`);
          break;
        }
        if (toAgentId === api.getAgentId()) {
          console.warn(`[${name}] SEND_DM target is self. Skipping.`);
          break;
        }
        const sent = await api.sendDM(toAgentId, dmMessage);
        if (!sent) {
          return `SEND_DM failed: unable to deliver DM to ${toAgentId}`;
        }
        console.log(`[${name}] Sent DM to ${toAgentId}: "${dmMessage.slice(0, 50)}..."`);
        break;
      }
      ...
      case 'START_CERTIFICATION': {
        ...
        if (!templateId) {
          console.warn(`[${name}] START_CERTIFICATION requires templateId. Skipping.`);
          break;
        }
        const certResult = await api.startCertification(templateId);
        const runId = certResult.run?.id || 'unknown';
        const deadline = certResult.workOrder?.deadlineAt
          ? new Date(certResult.workOrder.deadlineAt).toISOString()
          : 'unknown';
        console.log(`[${name}] Started certification run ${runId} for ${templateId} (deadline: ${deadline})`);
        // Store run ID on decision for memory tracking
      ...
      case 'SUBMIT_CERTIFICATION_PROOF': {
        ...
        if (!runId || !txHash) {
          console.warn(`[${name}] SUBMIT_CERTIFICATION_PROOF requires runId and txHash. Skipping.`);
          break;
        }
        const proofResult = await api.submitCertificationProof(runId, { txHash });
        const passed = proofResult.verification?.passed ?? false;
        console.log(`[${name}] Certification proof for run ${runId}: ${passed ? 'PASSED ✓' : 'FAILED ✗'}`);
        break;
      }

      case 'CHECK_CERTIFICATION': {
        const runs = await api.getCertificationRuns();
        const active = runs.filter(r => r.status === 'created' || r.status === 'active' || r.status === 'submitted' || r.status === 'verifying');
        const passed = runs.filter(r => r.status === 'passed');
        console.log(`[${name}] Certification runs: ${runs.length} total, ${active.length} active, ${passed.length} passed`);
        break;
      }

      case 'EXECUTE_SWAP': {
        // Legacy — kept for backward compat but agents should use EXECUTE_ONCHAIN now
        console.warn(`[${name}] EXECUTE_SWAP is deprecated. Use EXECUTE_ONCHAIN + APPROVE_TOKEN instead.`);
        break;
      }

      case 'EXECUTE_ONCHAIN': {
        // Pure sign-and-send — agent builds all calldata
        const to = typeof p.to === 'string' ? p.to.trim() : '';
        const data = typeof p.data === 'string' ? p.data.trim() : '';
        const valueRaw = p.value;
        if (!to || !data) {
          console.warn(`[${name}] EXECUTE_ONCHAIN requires to and data. Skipping.`);
          break;
        }
        if (!/^0x[a-fA-F0-9]{40}$/.test(to)) {
          console.warn(`[${name}] EXECUTE_ONCHAIN: invalid 'to' address. Skipping.`);
          break;
        }
        if (!/^0x[a-fA-F0-9]*$/.test(data)) {
          console.warn(`[${name}] EXECUTE_ONCHAIN: invalid 'data' hex. Skipping.`);
          break;
        }
        if (!chain) {
          console.warn(`[${name}] EXECUTE_ONCHAIN requires ChainClient (no private key configured). Skipping.`);
          break;
        }
        const value = valueRaw ? BigInt(String(valueRaw)) : 0n;
        console.log(`[${name}] Executing onchain tx: to=${to}, data=${data.slice(0, 10)}..., value=${value.toString()}`);
      ...
      case 'APPROVE_TOKEN': {
        ...
        const spender = typeof p.spender === 'string' ? p.spender.trim() : '';
        const amountRaw = p.amount;
        if (!token || !spender || !amountRaw) {
          console.warn(`[${name}] APPROVE_TOKEN requires token, spender, amount. Skipping.`);
          break;
        }
        if (!chain) {
          console.warn(`[${name}] APPROVE_TOKEN requires ChainClient (no private key configured). Skipping.`);
          break;
        }
        const amount = BigInt(String(amountRaw));
        console.log(`[${name}] Approving ${spender} to spend ${amount.toString()} of ${token}`);
